...
use openjoc_scene::{
    ProgrammeAudioSource, ProgrammeLayout, ProgrammeLayoutError, ProgrammeObjectClass,
};
...
fn optional_lfe_maps_outside_fifteen_dynamic_joc_rows() {
    ...
    assert_eq!(
        ...
        ProgrammeAudioSource::BaseLfe { channel_index: 0 }
    );
    assert_eq!(layout.bindings[0].class, ProgrammeObjectClass::Lfe);
    assert_eq!(
        layout.bindings[1].source,
        ProgrammeAudioSource::ReconstructionRow { row_index: 0 }
    );
    assert_eq!(
        layout.bindings[15].source,
        ProgrammeAudioSource::ReconstructionRow { row_index: 14 }
    );
    layout.validate_joc_output(15).expect("fifteen JOC rows");
    layout
        .validate_joc_output(15)
        .expect("fifteen structural reconstruction rows");
}
...
fn dynamic_only_without_lfe_is_one_to_one() {
    ...
    assert_eq!(
        layout.bindings[1].source,
        ProgrammeAudioSource::ReconstructionRow { row_index: 1 }
    );
    layout.validate_joc_output(2).expect("two JOC rows");
    layout
        .validate_joc_output(2)
        .expect("two structural reconstruction rows");
}
...
fn layout_rejects_unsupported_bed_and_lfe_source_failures() {
    ...
    assert_eq!(bed.validate_joc_output(1), Ok(()));
