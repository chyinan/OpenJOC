...
struct RenderJocArgs {
    ...
    layout: String,
    speaker_layout_definition: Option<SpeakerLayout>,
    output: PathBuf,
    binaural: bool,
    ...
    normalize_peak: Option<joc_render::PeakNormalization>,
    output_format: SampleFormat,
    no_progress: bool,
...
fn self_test(arguments: &[String]) -> Result<(), Box<dyn Error>> {
    ...
    println!("C_ABI        PASS (1.4-experimental)");
...
fn parse_render_joc(values: &[String]) -> Result<RenderJocArgs, Box<dyn Error>> {
    ...
    while index < values.len() {
        ...
        match flag.as_str() {
            ...
            "--layout-file" => layout_file = Some(PathBuf::from(value)),
            ...
            "--dialnorm" => dialnorm = parse_dialnorm_mode(value)?,
            "--normalize-peak" => {
                if normalize_peak.is_some() {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidInput,
                        "--normalize-peak may be supplied only once",
                    )
                    .into());
                }
                let target_dbfs = value.parse::<f64>().map_err(|_| {
                    io::Error::new(
                        io::ErrorKind::InvalidInput,
                        format!("invalid sample-peak normalization target {value}"),
                    )
                })?;
                normalize_peak = Some(joc_render::PeakNormalization::new(target_dbfs)?);
            }
            "--drc" => {
                if drc_mode.replace(value.clone()).is_some() {
                    return Err(io::Error::new(
    ...
    if layout.is_some() && layout_file.is_some() {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            "--layout and --layout-file are mutually exclusive; choose a preset or a custom geometry file",
        )
        .into());
    }
    if virtual_layout.is_some() && (layout.is_some() || layout_file.is_some()) {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
    ...
    if binaural && layout_file.is_some() {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            "--layout-file is for physical speaker output; binaural virtual layouts must use a preset",
        )
        .into());
    }
    ...
    let (layout, speaker_layout_definition) = if binaural {
        (
            virtual_layout
                .or(layout)
    ...
    } else if let Some(path) = layout_file {
        let custom = SpeakerLayout::from_json_file(path)?;
        (custom.name().to_owned(), Some(custom))
    } else {
        (layout.ok_or_else(usage_error)?, None)
    };
...
fn parse_dialnorm_mode(value: &str) -> Result<DialnormMode, io::Error> {
    match value {
        "default" | "DEFAULT" => Ok(DialnormMode::Default),
        "digital" | "DIGITAL" => Ok(DialnormMode::Digital),
        "analog" | "ANALOG" => Ok(DialnormMode::Analog),
        _ => Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            format!("unknown dialnorm mode {value}; expected default, digital, or analog"),
        )),
    }
}
...
fn render_joc_preflight(
    ...
    if !arguments.binaural {
        if let Some(layout) = &arguments.speaker_layout_definition {
            joc_render::validate_speaker_output_layout(layout, &arguments.output)?;
        } else {
...
fn run_legacy_render_pass(
    ...
    progress: &mut progress::ProgressReporter,
    timing_requested: bool,
) -> Result<LegacyPassReport, Box<dyn Error>> {
    ...
    if arguments.binaural {
        ...
        let result =
            eac3_decode::decode_internal_eac3_streaming_with_render_sink_and_policy_and_dialnorm(
                ...
                timing_requested.then_some(&mut decode_timing),
            );
        let (summary, reconstruction_tail) = result?;
        let tail = renderer.finish_with_reconstruction_tail(&reconstruction_tail)?;
        for block in tail {
            sink.consume(&joc_render::RenderedBlock {
                sample_rate: block.sample_rate,
                channels: vec![block.left, block.right],
                ...
                |frame_index, metadata, frame, base| {
                    ...
                    for block in blocks {
                        sink.consume(&block).map_err(|error| {
                            eac3_decode::DecodeEac3Error::Sink(error.to_string())
                        })?;
                    }
                    let stage_timings = renderer.take_stage_timings();
                    render_timing.bridge_control_assembly += stage_timings.bridge_control_assembly;
...
fn render_joc_with_peak_normalization(
    ...
    let normalization = arguments
        .normalize_peak
        .ok_or("normalization policy is missing")?;
    let config = PayloadDecoderConfig {
        reference_screen: None,
        oamd: OamdDecoderConfig::with_trim_configuration_count(arguments.trim_configuration_count),
    };
    ...
    if let Some(report) = performance.as_mut() {
        ...
        report.processed_access_units = timing.access_units;
        report.total_audio_samples = timing.samples;
        report.sample_rate_hz = Some(timing.sample_rate);
    }
    ...
    let mut output = if arguments.binaural {
    ...
    } else if let Some(layout) = &arguments.speaker_layout_definition {
        joc_render::JocPcmOutput::new_for_canonical_layout(
            &arguments.output,
            arguments.output_format,
            overwrite_authorized,
            layout,
        ...
        let layout = SpeakerLayoutPreset::for_name(&arguments.layout)?.semantic_channel_layout();
...
fn render_joc(
    ...
    if arguments.binaural {
        ...
        let mut output = if let Some(layout) = &arguments.speaker_layout_definition {
            joc_render::JocPcmOutput::new_for_canonical_layout(
                &arguments.output,
                arguments.output_format,
                overwrite_authorized,
        ...
        let result =
            eac3_decode::decode_internal_eac3_streaming_with_render_sink_and_policy_and_dialnorm(
                ...
                |frame_index, metadata, frame, base| {
                    ...
                    for block in blocks {
                        let output_start = collect_timing.then(std::time::Instant::now);
                        output.write_block(&block).map_err(|error| {
                            eac3_decode::DecodeEac3Error::Sink(error.to_string())
                        })?;
                        if let Some(output_start) = output_start {
                            render_timing.output_conversion_wav_write += output_start.elapsed();
...
fn render_joc_with_embedded_session(
    ...
    let config = OpenJocConfig {
        ...
        speaker_layout_definition: arguments.speaker_layout_definition.clone(),
        downmix,
        drc,
    ...
    let semantic_layout = if arguments.binaural {
        ...
        Some(layout.semantic_channel_layout())
    } else {
    ...
    let result = (|| -> Result<(), Box<dyn Error>> {
        ...
        let (selected_layout, output_layout, channel_count, lfe_index, lfe_count, channel_order) =
            if arguments.binaural {
                ...
                let (lfe_index, lfe_count) = arguments
                    .speaker_layout_definition
                    .as_ref()
                    .map_or((layout.lfe_index, layout.lfe_count()), |canonical| {
                        (canonical.lfe_index(), canonical.lfe_indices().len())
                    });
                (
                    layout.name.clone(),
...
const fn classify_joc_render_error(error: &joc_render::JocRenderError) -> CliErrorCategory {
    match error {
        ...
        | joc_render::JocRenderError::InvalidPeakNormalizationTarget(_) => {
        ...
        | joc_render::JocRenderError::NonFinitePcm => CliErrorCategory::DecodeFailure,
    }
}
...
mod profile_name_tests {
    use super::{
        DialnormMode, OverwriteDecision, SpatialContributionMode, TerminalCapabilities,
        ValidationProfileRequest, decide_overwrite, overwrite_answer_is_affirmative,
        parse_render_joc, parse_validation_profile, planned_render_outputs, render_joc_preflight,
    ...
    #[test]
    fn render_joc_dialnorm_and_peak_normalization_are_explicit_and_distinct_from_drc() {
        let base = [
            "input.m4a".to_owned(),
            "--layout".to_owned(),
            "2.0".to_owned(),
        ...
        let parsed = parse_render_joc(&base).expect("default policy");
        assert_eq!(parsed.dialnorm, DialnormMode::Default);
        assert!(parsed.normalize_peak.is_none());
        ...
        let explicit = [
            ...
            "--dialnorm".to_owned(),
            "analog".to_owned(),
            "--normalize-peak".to_owned(),
            "-0.1".to_owned(),
        ];
        let parsed = parse_render_joc(&explicit).expect("explicit output policies");
        assert_eq!(parsed.dialnorm, DialnormMode::Analog);
        assert_eq!(
            parsed.normalize_peak.expect("peak policy").target_dbfs(),
            -0.1
        );
        assert!(
            parse_render_joc(&[
    ...
    #[test]
    fn render_joc_layout_file_is_distinct_from_preset_layout_and_preserves_order() {
        let root = std::env::temp_dir().join(format!(
            "openjoc-layout-file-{}-{}.json",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .expect("clock")
        ...
        fs::write(
            &root,
            r#"{"version":1,"name":"cli-studio","speakers":[
                {"name":"A","azimuth":-40.0,"elevation":0.0},
                {"name":"B","azimuth":5.0,"elevation":8.0},
                {"name":"Sub","azimuth":0.0,"elevation":-20.0,"role":"lfe"}
            ]}"#,
        )
        .expect("layout file");
        let parsed = parse_render_joc(&[
            "input.m4a".to_owned(),
            "--layout-file".to_owned(),
            root.to_string_lossy().into_owned(),
            "--output".to_owned(),
            "out.caf".to_owned(),
        ])
        .expect("custom layout file");
        assert_eq!(parsed.layout, "cli-studio");
        assert_eq!(
            parsed
                .speaker_layout_definition
                .as_ref()
                ...
                .channel_labels(),
            &["A".to_owned(), "B".to_owned(), "Sub".to_owned()]
        );
        assert!(
            parse_render_joc(&[
                "input.m4a".to_owned(),
