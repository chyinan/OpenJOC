...
5. Record `logical_lfe_channels: 0` for Stereo and `logical_lfe_channels: 1` for each of the six multichannel candidates; provide no physical-subwoofer-count field and no friendly-name-derived semantics. Mark evidence-rule code `# pattern: Functional Core`.
