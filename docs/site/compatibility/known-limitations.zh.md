...
- Rust `OpenJocSession` 的数据包接口每次接收一个完整的 JOC 访问单元：I0 加可选的 D0。I0 可以使用 E-AC-3 语法，也可以在与 E-AC-3 D0 配对时使用 ETSI TS 102 366 Annex J 允许的原始 AC-3 语法（`bsid` 6 或 8）。解复用、任意字节拆分，以及一次传入多个访问单元，属于有界的 C 流式解码器或框架适配器负责的范围。
