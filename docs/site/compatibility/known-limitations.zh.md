...
- 标准 flat-7.X 仅由 JOC downmix index 1、七个 JOC 输入和 `L R C Ls Rs Lrs Rrs` Table-47 组装共同识别。JOC reconstruction 使用 I0+D0..Dn 组装后的七输入 plane。已通过 common-profile admission 的 Stereo 仍使用独立 I0 compatibility presentation；当组装后的 Flat-7.X plane 是 Stereo 的选定输入时，规范矩阵会以现有同侧 surround 规则接纳独立的 Lrs/Rrs rear-surround 输入，不会把它们重命名为 Ls/Rs。
...
- C ABI 1.5 在 OpenJOC 0.x 期间仍是实验性的。公共头文件、结构体大小、所有权规则、数值状态码和兼容初始化函数共同构成接口约定；ABI 仍可能演进。
