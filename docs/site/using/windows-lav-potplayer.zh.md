...
下面记录的是 v0.12 及以后 Windows 集成中验证过的安装包行为；本站的发布基线是 v0.16.0。
...
规范的 9.1.6 中间声道顺序为 `FL, FR, FC, LFE, Lb, Rb, Ls, Rs, Lw, Rw,
Ltf, Rtf, Ltm, Rtm, Ltr, Rtr`。LFE 遵循现有的显式 LFE 策略，其余 15 个
虚拟声道通过现有的精确查找或有界插值路径解析，再卷积为两个耳道。这是
虚拟布局，不是发送给下游的 16 声道扬声器输出。
