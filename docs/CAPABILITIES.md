...
| Binding | Reconstructed dynamic ADM Objects from decoded JOC Objects | `ADMITTED_WITH_SCOPE` | Bounded metadata preflight, typed binding profile, deterministic position-block export, strict/best-effort policy tests, independent ADM validation | Generated Objects carry decoded OAMD movement only within the exact profile; unsupported profiles remain neutral or reject |
| Binding | Unvalidated JOC binding profiles | `UNRESOLVED` | Explicit admission rejection and report reason | Bed, ISF, alternate-LFE, count/order-mismatched, compatibility, incomplete-Base-LFE, and other unvalidated profiles are not dynamically bound |
