# OpenJOC 0.5.0 capabilities

This is the canonical current capability status for the OpenJOC 0.5.0
...
| JOC rendering | Real supported E-AC-3 JOC to preset speaker WAV/CAF workflow | `ADMITTED_WITH_SCOPE` | `render-joc` decoder/bridge/output integration tests, automatic bridge-control assembly tests, 2.0 topology and Lo/Ro/Lt/Rt numeric tests, preset geometry, topology/count/LFE/order/mask/semantic-CAF checks, synthetic arbitrary-layout and 24-channel bridge tests | Experimental 2.0 plus 5.1, 5.1.2, 5.1.4, 7.1, 7.1.2, 7.1.4, 7.1.6, 9.1, 9.1.2, 9.1.4, and 9.1.6 paths; 2.0 uses generic object projection plus public E-AC-3 stereo Base matrices, optional metadata-gated LFE fold-down, and rejects unsupported Base back/height channels; all use one generic full-XYZ data-driven projector and separate LFE ownership; 7.1.6 and the 9.1 family are semantic CAF-only because their richer identities have no exact standard WAV mask bits; `CONTROL.json` is optional and remains a complete explicit override/test input; 22.2 remains outside the public preset boundary; generic library layouts remain supported; no authored-object binding or vendor-fidelity claim |
| JOC rendering | Real supported E-AC-3 JOC to stereo SOFA-backed binaural WAV | `ADMITTED_WITH_SCOPE` | Virtual-speaker integration tests, exact HRIR identity, delay-aligned spherical interpolation, azimuth-wrap and sparse-data tests, sample-rate preflight, direct-reference equivalence, partitioned equivalence, LFE policy, reset, and tail tests | Uses the default virtual field `7.1.4` unless `--virtual-layout` is supplied; caller-supplied admitted `SimpleFreeFieldHRIR` SOFA must provide exact or safely interpolatable directions for every non-LFE virtual speaker; CLI defaults LFE to `exclude`, with explicit `equal-power-dual-mono` available; output is always two-channel OpenJOC speaker virtualization, not a vendor/direct-object binaural claim |
...
| Release | OpenJOC 0.5.0 platform release assets | `ADMITTED_WITH_SCOPE` | Tagged GitHub Actions source/version checks, native platform quality gates, bundle verification, and aggregate checksum verification | Release workflow targets macOS arm64, Windows x86_64, and GNU/Linux x86_64; the macOS asset is ad-hoc signed and not notarized; local RC evidence covers macOS only |
...
openjoc render-joc FILE [--topology TOPOLOGY.json] --layout LAYOUT --output OUTPUT.wav [--binaural --sofa HRTF.sofa --virtual-layout LAYOUT | --binaural-sofa HRTF.sofa] [--lfe-policy exclude|equal-power-dual-mono]
openjoc --version
```
