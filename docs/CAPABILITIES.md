...
| Rendering | Static explicit-source binaural direct-FIR renderer | `ADMITTED_WITH_SCOPE` | `openjoc-render` exact-direction HRIR/provider validation, independent full-convolution oracle, ear-order, history, tail, reset, failure-atomicity, and input/tail partition tests | Caller supplies finite equal-length HRIR taps and exact static directions; fixed listener orientation, direct causal f64 FIR only; no SOFA, interpolation, moving source, partitioned convolution, room, distance, HRTF database, or JOC bridge |
