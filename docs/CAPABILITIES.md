# OpenJOC 0.9.2 capabilities

This is the canonical capability status for OpenJOC 0.9.2. Real-media
...
| Release | OpenJOC 0.9.2 platform assets | `ADMITTED_WITH_SCOPE` | GitHub Actions source/version checks, native platform quality gates, bundle verification, C ABI artifact checks, aggregate checksum verification, and completed real-media Logic/re-export/DEE workflow | Workflow targets macOS arm64, Windows x86_64, and GNU/Linux x86_64; archives include the CLI, public docs, `openjoc.h`, platform C ABI libraries, and OpenJOC Player Bundles; the macOS assets are ad-hoc signed and not notarized; direct DEE ingest of byte-exact OpenJOC ADM remains unsupported/unclaimed |
