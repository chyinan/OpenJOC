...
This is the canonical candidate capability status for OpenJOC 0.9.2. OpenJOC
0.9.1 remains the current public release until real-media acceptance. The
...
| Interchange | Reconstructed ADM/BW64 export | `ADMITTED_WITH_SCOPE` | Production bounded-memory compressed-media preflight/direct writer, seek-based validator, transactional cleanup, deterministic ADM XML/BW64 `ds64`/`axml`/`chna`, signed 24-bit PCM, mapping report, synthetic fixture, scale high-watermarks, and strict unresolved-binding rejection | Compressed input scales with one bounded AU; explicit JSON/scene inputs retain their diagnostic model; reconstructed neutral signals only while binding is unresolved; not original ADM master recovery or DAW certification |
...
| Release | OpenJOC 0.9.2 candidate platform assets | `ADMITTED_WITH_SCOPE` | GitHub Actions source/version checks, native platform quality gates, bundle verification, C ABI artifact checks, and aggregate checksum verification | Publication remains blocked on real-media ADM acceptance; workflow targets macOS arm64, Windows x86_64, and GNU/Linux x86_64; archives include the CLI, public docs, `openjoc.h`, platform C ABI libraries, and OpenJOC Player Bundles; the macOS assets are ad-hoc signed and not notarized |
