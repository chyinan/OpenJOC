...
| JOC rendering | Real supported E-AC-3 JOC to preset speaker WAV workflow | `ADMITTED_WITH_SCOPE` | `render-joc` decoder/bridge/output integration tests, automatic bridge-control assembly tests, preset geometry, topology/count/LFE/order/mask checks, synthetic arbitrary-layout and 24-channel bridge tests | Experimental 5.1, 5.1.2, 5.1.4, 7.1, 7.1.2, and 7.1.4 paths; all use one generic full-XYZ data-driven projector and separate LFE ownership; `CONTROL.json` is optional and remains a complete explicit override/test input; 2.0, 9.1.4, 9.1.6, and 22.2 remain outside the public preset boundary; generic library layouts remain supported; no authored-object binding or vendor-fidelity claim |
