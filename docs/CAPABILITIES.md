...
| Release | OpenJOC 0.12.0 platform assets and LAV integration | `ADMITTED_WITH_SCOPE` | GitHub Actions source/version checks, native platform quality gates, bundle verification, C ABI artifact checks, aggregate checksum verification, completed real-media Logic/re-export/DEE workflow, and the layered DirectShow/LAV transport and endpoint evidence above | Workflow targets macOS arm64, Windows x86_64, and GNU/Linux x86_64; the Windows DirectShow/LAV subset is limited to the seven explicit fixed policies; physical multichannel hardware playback and automatic semantic negotiation are not claimed; direct DEE ingest of byte-exact OpenJOC ADM remains unsupported/unclaimed |
