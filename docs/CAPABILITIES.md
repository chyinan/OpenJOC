...
| Rendering | Explicit-scene stereo and general 2D speaker renderer | `ADMITTED_WITH_SCOPE` | `openjoc-render` independent stereo/VBAP oracle, layout, continuity and block-streaming tests | Caller-supplied mono sources only; arbitrary validated horizontal layouts and adjacent-pair panning; no 3D/elevation, LFE, JOC bridge, HRTF, room model, or Dolby renderer-fidelity claim |
