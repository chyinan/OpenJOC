...
| TS 102 366 Annex E.1.2 | E-AC-3 syncframe syntax, size and timing | `openjoc-eac3` | bounded acquisition header, conditional BSI/addbsi extraction, option-4 mixdata bound, complete `audfrm` state, and first-`audblk` traversal through exact skip-field retention pass; normative bit allocation, mantissas, and later blocks remain | implemented |
| TS 102 366 6.2, E.2.4.3 | Fixed-point decoder bit allocation | `openjoc-eac3` | exhaustive exponent-to-PSD, Table 6.14 log-addition/integration, Tables 6.6-6.16 parameter/band/masking/pointer mappings, excitation/masking/delta/BAP primitives pass; mantissa traversal and audio-block integration remain planned | implemented |
