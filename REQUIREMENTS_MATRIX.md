...
| TS 103 420 8.2-8.3, TS 102 366 E.1.2/E.1.2.5/E.1.3.1.2 | External multi-fixture carrier census | `openjoc-cli`, `openjoc-eac3` | gitignored or environment-selected manifest; checked hashes; deterministic per-fixture JSON/text reports; bounded frame-end attempts; explicit examined versus unresolved carrier states; no committed programme bytes | implemented |
| TS 102 366 E.1.2/E.2.8.2 | Parse-only audio-block carrier traversal | `openjoc-eac3` | BSI→audfrm→first audblk prefix and declared `skipfld` reached without PCM synthesis; later block starts remain explicitly unresolved until normative mantissa cursor traversal is proven | implemented |
