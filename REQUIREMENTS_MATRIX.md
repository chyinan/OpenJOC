...
| TS 102 366 E.1.2/E.2.8.2 | Parse-only audio-block carrier traversal | `openjoc-eac3` | BSI→audfrm→all six `audblk` prefixes and declared `skipfld` reached on the four external fixtures without PCM synthesis; grouped mantissa cursor traversal reaches every block with zero malformed/unresolved counts; unsupported syntax remains explicit | implemented |
